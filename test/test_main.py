...
def test_upload_file():
    test_file_path = os.path.join(os.path.dirname(__file__), "Tts Test Audiobook.pdf")
    with open(test_file_path, "rb") as f:
        file_data = {"file": ("Tts Test Audiobook.pdf", f, "application/pdf")}
        response = client.post("/upload", files=file_data)
        assert response.status_code == 200
        assert response.json()["filename"] == "Tts Test Audiobook.pdf"
